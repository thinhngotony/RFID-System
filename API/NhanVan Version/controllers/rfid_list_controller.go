...
func GetJanCodefromRFIDList(c *gin.Context) {
	...
	if valid_rfid, invalid_rfid := db_client.GetDataFromRFIDList(db, reqBody.RFIDList); valid_rfid != nil && invalid_rfid == nil {
		...
		c.JSON(http.StatusOK, gin.H{
			...
			"data":    valid_rfid,
			"error":   invalid_rfid,
			"message": "RFID not exists in database",
		})
...
func GetJanCodefromRFIDList_2(c *gin.Context) {
	var reqBody RFIDList
	...
	if valid_rfid, invalid_rfid := db_client.GetDataFromRFIDList_2(db, reqBody.RFIDList); valid_rfid != nil && invalid_rfid == nil {
		c.JSON(http.StatusOK, gin.H{
			"code":    utils.SUCCESSFULLY_CODE,
			"data":    valid_rfid,
			"error":   nil,
