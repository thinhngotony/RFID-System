...
func InsertDataToMasterTable(c *gin.Context) {
	...
	if data_exist, _, _ := db_client.CheckExistRFID(db, []string{reqBody.Drgm_rfid_cd}); len(data_exist) == 0 {
		if create_date, shop_code, err := db_client.InsertMaster(db,
			...
			reqBody.Drgm_media_cd,
			reqBody.Drgm_product_id); err != nil {
			c.JSON(http.StatusOK, gin.H{
...
func DeleteDataFromMasterTable(c *gin.Context) {
	...
	rfid_list, empty_rfid, _ := db_client.CheckExistRFID(db, reqBody.RFIDList)
