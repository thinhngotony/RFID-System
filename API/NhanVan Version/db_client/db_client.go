...
type Data_Search struct {
	...
	Drgm_product_id      *string  `json:"drgm_product_id"`
...
func ConvertFromRFID_2(db *sql.DB, rfid string) ([]RFID_Jan_2, error) {
	log.Printf("Getting JAN code from list of RFID")
	query := `select drgm_rfid_cd, drgm_jan, drgm_jan2, drgm_cost_price, drgm_price_tax_off, drgm_goods_name, drgm_artist from drfid_rfgoods_master where drgm_rfid_cd = ?;`
	ctx, cancelfunc := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancelfunc()
	stmt, err := db.PrepareContext(ctx, query)
	if err != nil {
	...
	var rfid_list = []RFID_Jan_2{}
	for rows.Next() {
		var prd RFID_Jan_2
		if err := rows.Scan(&prd.RFID, &prd.JanCode_1, &prd.JanCode_2, &prd.Drgm_cost_price, &prd.Drgm_price_tax_off, &prd.Drgm_goods_name, &prd.Drgm_artist); err != nil {
			log.Printf("Vô 3")
			return []RFID_Jan_2{}, err
		}
		rfid_list = append(rfid_list, prd)
...
func CheckExistRFID(db *sql.DB, rfid_list []string) ([]string, []string, error) {
...
func GetDataFromRFIDList_2(db *sql.DB, RFIDList []string) ([]RFID_Jan_2, []string) {
	var data []RFID_Jan_2
	var empty_rfid []string
	RFIDListValid := RemoveDuplicateStr(RFIDList)
	for i := 0; i < len(RFIDListValid); i++ {
		if rfid, _ := ConvertFromRFID_2(db, RFIDListValid[i]); len(rfid) == 0 {
			empty_rfid = append(empty_rfid, RFIDListValid[i])
		} else {
			data = append(data, rfid...)
		}
...
// func InsertMultiRowsToLogTable(db *sql.DB, rfid_list []string, mode []string) (bool, string, string, error) {
...
// 	Shop_code := utils.LoadConfig(utils.ADDRESS).ShopCode
// 	create_date := time.Now().Format(utils.TIME_FORMAT)
// 	query := `INSERT INTO drfid_taglog(dt_create_date, dt_com_shop_cd, dt_rfid_cd, dt_mode, sync_flag) VALUES (?` + strings.Repeat(",?", len(args)-1) + `,0);`
// 	ctx, cancelfunc := context.WithTimeout(context.Background(), 5*time.Second)
// 	defer cancelfunc()
...
// 	_, err = tx.ExecContext(ctx, query, create_date, Shop_code, rfid_list, mode)
// 	if err != nil {
...
// func (cs Respond) bulkInsert(db *sql.DB) error {
...
// 			index*5+5,
// 		))
// 		vals = append(vals, create_date, Shop_code, contact.RFID, contact.Mode, 0)

// 		fmt.Println("Cái cần xem là ", vals)
// 	}

// 	txn, err := db.Begin()
// 	if err != nil {
// 		log.Printf("Error %s could not start a new transaction", err)

// 	}

// 	insertStatement := fmt.Sprintf("INSERT INTO drfid_taglog(dt_create_date, dt_com_shop_cd, dt_rfid_cd, dt_mode, sync_flag) VALUES %s", strings.Join(placeholders, ","))

// 	fmt.Println(insertStatement)
// 	_, err = txn.Exec(insertStatement, vals...)
// 	if err != nil {
// 		txn.Rollback()
// 		log.Printf("Error %s failed to insert multiple records at once", err)

// 	}

// 	if err := txn.Commit(); err != nil {
// 		log.Printf("Error %s failed to commit transaction", err)

// 	}

// 	log.Printf("Thành công rồi")

// 	return nil
// }
// func InsertMaste11r(db *sql.DB) {
...
func InsertMaster(db *sql.DB,
	...
	drgm_cost_price int,
	drgm_media_cd string,
	drgm_product_id string) (string, string, error) {

	log.Printf("Importing data to drfid_rfgoods_master table")
	Shop_code := utils.LoadConfig(utils.ADDRESS).ShopCode
	create_date := time.Now().Format(utils.DATE_FORTMAT)
