...
type Insert_Raw_Data struct {
	ApiKey string                       `json:"api_key" validate:"required"`
	Data   []*db_client.Detail_Raw_Data `json:"data" validate:"required"`
}
...
func GetSmartSelfSetting(c *gin.Context) {
	...
	db, _ := db_client.DbConnection_smartshelf()
	defer db.Close()
...
func InsertRawData(c *gin.Context) {
	var reqBody Insert_Raw_Data
	...
	for i, _ := range reqBody.Data {
		isSuccess, _ = db_client.InsertDataRaw(db, *reqBody.Data[i])
	}
