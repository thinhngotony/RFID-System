...
type Detail_Raw_Data struct {
	Rfid      string `json:"rfid" validate:"required"`
	RSSI      int    `json:"rssi" validate:"required"`
	Shelf_no  string `json:"shelf_no" validate:"required"`
	Antena_no int    `json:"antena_no" validate:"required"`
}

func DbConnection_smartshelf() (*sql.DB, error) {
...
func InsertDataRaw(db *sql.DB, req Detail_Raw_Data) (bool, error) {
	log.Printf("Importing data to drfid_raw_data table")
	// RSSI_float, _ := strconv.ParseFloat(req.RSSI, 32)
	RSSI_float := float64(req.RSSI)
	m_10 := 100 - math.Abs(RSSI_float)

	query := `INSERT INTO drfid_raw_data(drd_rfid_cd, drd_anten_no, drd_rssi, drd_shelf_no, m_10, n_10 ) VALUES (?, ?, ?, ?, ?, ?);`
	ctx, cancelfunc := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancelfunc()
	stmt, err := db.PrepareContext(ctx, query)
	if err != nil {
	...
	defer stmt.Close()
	res, err := stmt.ExecContext(ctx, req.Rfid, req.Antena_no, m_10, req.Shelf_no, m_10, req.Antena_no)
	if err != nil {
		log.Printf("Error %s when inserting row into drfid_raw_data table", err)
		return false, err
...
func GetSmartSelfLocationByCol(db *sql.DB, shelf_name string, col int, row int) ([]Get_SmartSelf_Location, bool, error) {
	log.Printf("Getting setting of smart shelf")
	query := `select 
	EPC,
	jancode,
	product_name,
