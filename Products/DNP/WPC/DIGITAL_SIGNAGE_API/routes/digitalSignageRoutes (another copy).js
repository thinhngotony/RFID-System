    ...
    if(rfidTagTypes && rfidTagTypes.length > 0 && rfidTagTypes[0].dst_sheft_type == "SHELF") {
        ...
        if(janTypes && janTypes.length > 0) {
            ...
            if(autoReq) {
                ...
                result = await bookInfoTagHandle(rfid, req.io, autoReq);
            }
        }
    }
...
async function specialTagHandle(io, rfidTagType, displayTitle, shelfName, rfid, autoReq) {
    try {
        ...
        FROM 
            ...
            WHERE 
                ifnull(pos.dpp_rfid_cd, '') != '' AND
                dpp_shelf_name = "${shelfName}" AND
                dpp_rfid_cd in (
                    SELECT dlm_rfid_cd 
                    FROM drfid_log_move lm 
                    WHERE 
                        dlm_cnt = (select max(dlm_cnt) FROM drfid_log_move where dlm_rfid_cd = lm.dlm_rfid_cd)
            GROUP BY 
                dpp_rfid_cd) as temp
        ORDER BY
                temp.shelf_pos, temp.shelf_col_pos  `);
        if(result.length > 0) {
            ...
            for (const iterator of result) {
                if(iterator.img == null || iterator.img == "") {
                    let images = await getImage([iterator.isbn]);
                    if (images != null && images.length > 0) {
                        iterator.img = images[iterator.isbn] ?? "";
                    } else {
                        iterator.img = "";
                    }
                } else {
                    iterator.img = "data:image/jpeg;base64," + iterator.img;
                }
...
async function bookInfoTagHandle(rfidTag, io, autoReq) {
    try {
        ...
        if(result.length == 0 || result[0].jan == null || result[0].jan == "") {
            return { success: false, message: "no data" };
        }

        await axios.get(process.env.GCE_HOST + '/api/product-info?jan=' + result[0].jan)
            .then(async response => {
                if(response.data.success) {
                    if(response.data.data == null) {
                        ...
                        io.emit("displayScreen", { type: "BOOK_INFO", data : [response.data.data], rfid: rfidTag, autoReq: autoReq });
                        console.log( { type: "BOOK_INFO", data : response.data, rfid: rfidTag});
                    }
                } else {
            ...
            .then(async response => {
                if(response.data.success) {
                    if(response.data.data == null) {
                        ...
                        await axios.get(process.env.GCE_HOST + '/api/best-seller?mc3=' + response.data.data.bqct_category_cd1)
                            .then(async response => {
                                if(response.data.success) {
                                    if(response.data.data == null) {
                                        io.emit("displayScreen", { type: "DIGITAL_SIGNAGE", data : [], rfid: rfidTag, autoReq: autoReq });
                                        ...
                                        var cCodeInfo = "<br>" + response.data.data[0].category_name_1 
                                        + "<br>" + response.data.data[0].category_name_2 
                                        + "<br>" + response.data.data[0].category_name_3;
