...
router.post("/rfid-scan", async (req, res, next) => {
    ...
    if(rfidTagTypes && rfidTagTypes.length > 0 && rfidTagTypes[0].dst_sheft_type == "SHELF") {
        ...
        result = await specialTagHandle(req.io, rfidTagTypes[0].dst_type, rfidTagTypes[0].dst_display_title, rfidTagTypes[0].dst_shelf_name, rfid);
...
async function specialTagHandle(io, rfidTagType, displayTitle, shelfName, rfid) {
    try {
        ...
        if(result.length > 0) {
            ...
            io.emit("displayScreen", { type: "SMART_SHELF", title: displayTitle, data : response, rfid: rfid });
        } else {
...
async function getImage(isbns) {
    try {
        ...
        for (let index = 0; index < isbns.length; index++) {
            ...
            hashObj[isbns[index]] = result.data?.base64 ?? result.data?.cover;
        }
        return hashObj;
    } catch (err) {
        console.log(err);
